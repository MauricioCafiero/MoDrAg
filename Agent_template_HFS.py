...
def tool1_node(state: State) -> State:
  '''
    tool doc string
  '''
  print("tool1")
...
#for the tools
builder.add_node("tool1_tool", tool1_node)

builder.add_edge(START, "first_node")
builder.add_conditional_edges("first_node", get_chemtool, {
    "tool1_tool": "tool1_node",
    None: "parser_node"})
...
builder.add_edge("tool1_node", "loop_node")
...
@spaces.GPU
def Agent(task, smiles): # add variables as needed
  ...
  input = {
    "messages": [
        HumanMessage(f'query_smiles: {smiles}, query_task: {task}') # add variables as needed
    ]
  }
  #print(input)
...
with gr.Blocks(fill_height=True) as forest:
  ...
  with gr.Row():
    ...
    with gr.Column():
      ...
      calc_btn.click(Agent, inputs = [task, smiles], outputs = [props, pic])
      task.submit(Agent, inputs = [task, smiles], outputs = [props, pic])
